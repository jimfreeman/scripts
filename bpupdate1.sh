...
echo mysql -h $db_host -u$db_user -p$db_pass onapp -e "UPDATE user_groups_billing_plans set billing_plan_id=$user_plan_id"
...
nohup mysql -h $db_host -u$db_user -p$db_pass onapp -e "DELETE FROM billing_plans where id NOT IN ($user_plan_id,2,3)" > /dev/null 2>&1 && echo " $icount Billing Plans Deleted" || echo "There was a problem"
