...
nohup mysql -h $db_host -u$db_user -p$db_pass onapp -e "UPDATE users set billing_plan_id=$user_plan_id where id NOT IN (2)" > /dev/null 2>&1 && echo "All Users have been updated" || echo "Users have not been updated"
echo -n "Do you want to clean up all other billing plans? [y/n] "
read delete_user_plans
if echo "$delete_user_plans" | grep -iq "^y" ;then
nohup mysql -h $db_host -u$db_user -p$db_pass onapp -e "DELETE FROM billing_plans where id NOT IN ($user_plan_id,2,3)" > /dev/null 2>&1 && echo "All Billing Plans have been cleaned up" || echo "Billing Plans have not been cleaned up"
nohup mysql -h $db_host -u$db_user -p$db_pass onapp -e "UPDATE count_resources SET count= (SELECT COUNT(*) FROM billing_plans) where resource_type='billing_plans'" > /dev/null 2>&1 && echo "Billing Plan counts have been updated" || echo "Billing Plan counts have not been updated"
else
echo "OK, no problem!"
fi
